...
void decoder_CUDA(double * x, double * y, int * genotipo)
{
    ...
    *x = *y = 0.0;
    ...
    // calculo del primer decimal
    for(i=0; i<LONG_COD/2; i++)
        *x += genotipo[i] * (1<<((unsigned int)((LONG_COD/2)-(i+1))));
    *x = (*x) * INTERVALO + LIMITE;
 
    //calculo del segundo decimal
    for(;i<LONG_COD;i++)
        *y += genotipo[i] * (1<<((unsigned int)((LONG_COD/2)-(i+1))));
    *y = (*y) * INTERVALO + LIMITE;
}
...
__global__ void seleccionTorneos_CUDA(Individuo * poblacion_act,Individuo * next_generation){
	...
	Individuo a = poblacion_act[(int) (((double) DPOBLACION)*my_rand(&seedone)/(RAND_MAX+1.0))];
	Individuo b = poblacion_act[(int) (((double) DPOBLACION)*my_rand(&seedtwo)/(RAND_MAX+1.0))];
    ...
    if((double) my_rand(&seed)/(RAND_MAX+1.0) < PROB_CRUCE)
        {
            ...
            seleccion[tid].aptitud = x*x+y*y;
 
            decoder_CUDA(&x, &y, seleccion[tid+1].genotipo);
            seleccion[tid+1].aptitud = x*x + y*y;
        }
}
...
void AG (void)
{
    ...
    do
    {
	seleccionTorneos_CUDA<<<POBLACION/THREADS_PER_BLOCK,THREADS_PER_BLOCK>>>(d_poblacion,d_seleccion);
    cruzarSeleccion_CUDA<<<(POBLACION/THREADS_PER_BLOCK)/2,THREADS_PER_BLOCK>>>(d_seleccion);
